...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation
{
    ...
    NSString *longitude = [NSString stringWithFormat:@"%f", manager.location.coordinate.latitude];
    
    NSDictionary *childDict = @{@"utf8": @"✓", @"authenticity_token":@"EvZva3cKnzo3Y0G5R3NktucCr99o/2UWOPVAmJYdBOc=", @"user":@{@"username":self.parentUserID,@"latitude":latitude,@"longitude":longitude}, @"commit":@"Create User", @"action":@"update", @"controller":@"users"};
    ...
    if ([NSJSONSerialization isValidJSONObject:childDict]) {
        ...
        //NSString *childDictJSONString = [[NSString alloc] initWithData:self.childDictJSON encoding:NSUTF8StringEncoding];

...
- (IBAction)verifyChildLocation:(id)sender {
    ...
    NSString *digitalLeashURLString =
    
        [NSString stringWithFormat:@"http://protected-wildwood-8664.herokuapp.com/users/%@.json", self.userID];
          //  [NSString stringWithFormat:@"http://localhost:8200/users/%@.json", self.userID];
    
    NSURL *digitalLeashUserURLGet = [NSURL URLWithString:digitalLeashURLString];
    
    NSMutableURLRequest *requestVerification = [NSMutableURLRequest requestWithURL:digitalLeashUserURLGet];
    
    [requestVerification setHTTPMethod:@"GET"];
    
    
    [NSURLConnection connectionWithRequest:requestVerification delegate:self];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if (submitClicked) {
        ...
        [self receiveUserDataUsingGet];
    }
    
}

-(void) receiveUserDataUsingGet {
    
    NSString *jsonString = [[NSString alloc] initWithData:_responseData encoding:NSUTF8StringEncoding];
    NSData *jsonDataFromString = [jsonString dataUsingEncoding:NSUTF8StringEncoding];
    
    if (jsonDataFromString != nil) {
        NSError *jsonError;
        
        NSDictionary *jsonDictionary = [NSJSONSerialization JSONObjectWithData:jsonDataFromString options:NSJSONReadingMutableContainers error:&jsonError];
        ...
        if (!jsonDictionary) {
            ...
            if ([[jsonDictionary objectForKey:@"is_in_zone"] isEqual:@1]) {
                isInTheZone = YES;
            } else {
                isInTheZone = NO;
            }
